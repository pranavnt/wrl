...
def _unpack(batch):
    """Unpack stacked pixel observations into obs / next_obs slices.

    The memory-efficient replay buffer stores `num_stack+1` frames per
    pixel key in `observations` and omits `next_observations[<pixel>]` —
    here we recover the next-obs slice.
    """
...
def load_resnet10_params(agent, image_keys=("image",), public=True):
    """Load pretrained ResNet-10 params from the hil-serl release into an agent."""
    ...
    if not public:
        ...
        if os.path.exists(file_path):
            ...
            url = (
                "https://github.com/rail-berkeley/serl/releases/download/"
                f"resnet10/{file_name}"
            )
            ...
            if total_size != 0 and t.n != total_size:
                raise RuntimeError("ResNet-10 download size mismatch")
